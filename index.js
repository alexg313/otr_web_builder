...
function minifySass() {
  return gulp
    .src(options.baseDir + "/**/*.scss")
    .pipe(sass.sync())
    .pipe(
      cleanCSS({ debug: true, compatibility: "ie8" }, function (details) {
...
function compileTs() {
  return gulp
    ...
    .js.pipe(gulp.dest(options.buildDir));
}
...
function copyNodeModules(cb) {
  gulp
    .src(options.rootDir + "package.json")
    .pipe(gulp.dest(options.buildDir))
    .on("end", () => {
      let installEmptyGit = spawn(`cd ${options.buildDir} && git init`, {
        stdio: "inherit",
        shell: true,
      });
      let cmd = spawn(
        "npm",
        ["install", "--prefix", "./build/", "--only", "prod"],
        {
          stdio: "inherit",
          shell: true,
        }
